...
def everything_alright(tab):
    ...
    st.write(('user' in session) and session['verify'])
...
def display_salary():
    ...
    _col1.dataframe(pd.DataFrame(list(zip(list(salaryDf.columns), list(salaryDf.iloc[_index])))))
...
with payments:
    if everything_alright('payment records'):
        ...
        if role == 'Simple': display('Your Payment Records', paymentDf)
        ...
        # Else, Give Option to Add Payment Record
            ...
            with add:
                ...
                entries = []                                      # List to store records to be added
                session['last'] = len(get_sheet('Payments')) + 2  # Last filled index of sheet where new records to be added
                balanceDf = get_sheet('BalanceSheet')             # Loading Sheet to get old balances
                spread = Spread(spreadSheetName, client=client)
                single, upload = st.tabs(['Single', 'Upload Excel'])
                with single:
                    with st.form(key='addPayment', clear_on_submit=True):
                        name = st.selectbox('Name', list(employeeList))
                        amount = st.text_input('Amount')
                        date = st.date_input('Date')
                        confirm = st.form_submit_button('Confirm')
                    if confirm:
                        prev = update_df(name[-4:], int(amount))                                                     # Dataframe
                        entry = [name[-4:], date.strftime("%d.%m.%Y"), name[:-7], amount, prev, prev - int(amount)]  # Entry Array
                        spread.update_cells('A' + str(session['last']), 'F' + str(session['last']), entry)           # Adding Entry to Google Sheet
                        session['last'] += 1
...
with balances:
    if everything_alright('balance sheet'):
        ...
        if role == 'Simple':
            display('Your Balances', balanceDf)
        else:
